...
class BB extends PluginBase implements Listener {
	...
	{
		$this->getLogger()->notice(TE::AQUA . "BuildBattle by DanilZz");
		...
		foreach($this->arenas as $lev)
		{
			$this->getServer()->loadLevel($lev);
                ...
                $temas = array("Bird","Fish","Auto - Car","Dragon","Internet","Frog","Mario Bros","Dog","Mage","Tractor","Boat","WaterFall","Tiger","AirPlane","Superheroe","Soccer","MCPE Mobs-Monsters");
	...
	public function onInteract(PlayerInteractEvent $event)
	{
		...
		if($tile instanceof Sign) 
		{
			...
			{
				$tile->setText(TE::AQUA . "[Join]",TE::GREEN  . "0 / 16","§f" . $this->currentLevel,$this->prefix);
				$this->refreshArenas();
				$this->currentLevel = "";
				$this->mode = 0;
		...
		elseif(in_array($player->getName(), $this->op)&& $this->mode>=1 && $this->mode<=16)
		{
			...
			$player->sendMessage($this->prefix . "Spawn ".$this->mode." Registrado!");
			$this->mode++;
			$config->save();
		}
		...
		foreach($tiles as $t) {
			if($t instanceof Sign) {	
				...
				if($text[3]==$this->prefix)
				{
					...
					{
						$ingame = TE::DARK_PURPLE . "[Game running]";
					}
					elseif($aop>=16)
					{
						$ingame = TE::GOLD . "[Full]";
					}
                                        $t->setText($ingame,TE::GREEN  . $aop . " / 16",$text[2],$this->prefix);
				}
			}
		}
	}
					...
					else
					{
						...
						if($config->get($arena . "inicio")==1)
						{
							if($timeToStart>0)
							{
								...
								{
									$pl->sendTip(TE::WHITE."Start in ".TE::GREEN . $timeToStart . TE::RESET);
                                                                        if($timeToStart<=5)
                                                                        {
							...
							else
							{
                                                                ...
                                                                if($time>170)
                                                                {
                                                                    foreach($playersArena as $pla)
                                                                    {
                                                                        $pla->sendTip(TE::BOLD.TE::AQUA."Building: " .TE::GREEN. $tema .TE::AQUA. "  End in: ".TE::YELLOW.$minutes.TE::DARK_PURPLE.":".TE::YELLOW.$seconds. TE::RESET);
                                                                    }
                                                                }
								$time--;
								if($time == 469)
								{
									...
									foreach($playersArena as $pl)
									{
                                                                            ...
                                                                            $pl->sendMessage(TE::YELLOW."> ".TE::BOLD.TE::GREEN."Build".TE::LIGHT_PURPLE."Battle");
                                                                            $pl->sendMessage(TE::YELLOW."> ".TE::WHITE."Building: ".TE::BOLD.TE::AQUA. $tema);
                                                                            $pl->sendMessage(TE::YELLOW."> ".TE::WHITE."Lucky you!");
                                                                            $pl->sendMessage(TE::YELLOW."> ".TE::GREEN."You have ".TE::AQUA."5".TE::GREEN." minutes");
                                                                            $pl->sendMessage(TE::YELLOW.">--------------------------------");
